...
|`androidCoverage`| 完全地执行 instrumentation 类。它的值将传入至 `adb shell am instrument -e coverage true -w` 的 `-w` 中。| `com.my.Pkg/com.my.Pkg.instrumentation.MyInstrumentation`|
